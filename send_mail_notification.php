...
logTxt('POST_ID = '.$_POST["ID"],'send_mail_notification');
?>

<?php
$AllQuery = simular_vacancies( $_POST["ID"] );
$simularVacancies = '';
...
$mail->SMTPAuth = TRUE;
$mail->SMTPSecure = "tls";
$mail->Port     = get_field('smtp_port', 'option');
$mail->Username = get_field('email_username', 'option');
...
//$mail->AddAddress('a.dovgun@gmail.com');

$mail->Subject = "Ваше сообщение успешно доставлено";
/*Body empty*/
$mail->MsgHTML('
                                    ...
                                    <tbody>
                                      <tr>
                                        <td class="row-container" style="font-family: Arial, sans-serif;padding: 0;text-align: center;height: 100%;vertical-align: top;width: 100%"><!--[if mso]>
                                          ...
                                          Ваш отклик на позицию <b><a href="'.get_permalink($_POST["job_id"]).'">'.$_POST['job_title'].'</a></b> был направлен одному из Консультантов Hays.
...
if(!$mail->Send()) {
    echo "<p class='error'>".get_field('apply_answer_ok', 'option')."</p>";
	logTxt('POST_ID = '.$_POST["ID"].' не отправился email '.$_POST['email'],'send_mail_notification');
} else {
    echo "<p class='success'>".get_field('apply_answer_ok', 'option')."</p>";
    echo "<style>#formSending{display: none;}</style>";
	logTxt('POST_ID = '.$_POST["ID"].' отправился на '.$_POST['email'],'send_mail_notification');
}
