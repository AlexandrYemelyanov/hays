...
$request = getRequest();
define('HAYS_DIR', get_template_directory());
...
if (isset($request['path'])) {
    ...
    switch ($request['path']) {
        case "api/mindbox/events/":
            require_once (HAYS_DIR.'/api/mindbox/events.php');
            break;
    }
...
function tve_get_results()
{
    ...
    $q .= '
		...
		order by name';
    //echo $q;
    $specialismAll = $wpdb->get_results($q, ARRAY_A);
    foreach ($specialismAll as $key => $item) {
        $specialismAll[$key]['name'] = htmlspecialchars_decode($item['name']);
    }
    //print_r($city);
    ...
    /*Источник вакансии*/
    if ( ! empty($request['queryvars']['source'])) {
        $source = $request['queryvars']['source'];
        ...
        if ($source == 2) {
            $source = 'hays-response';
        }

        $query .= '
			INNER JOIN '.$wpdb->postmeta.' pmsite ON ( p.ID = pmsite.post_id ) and (pmsite.meta_key = "site") AND (pmsite.meta_value = "'.$source.'")
		';
    }
    ...
    foreach ($posts as $post) {
        ...
        if ( ! empty($current_specialism[0]->name)) {
            $specialism .= str_replace('&amp;', '&', $current_specialism[0]->name);
        }

        if ( ! empty($current_specialism[1]->name)) {
            $specialism .= ' | '.str_replace('&amp;', '&', $current_specialism[1]->name);
        }

        if ( ! empty($current_specialism[2]->name)) {
            $specialism .= ' | '.str_replace('&amp;', '&', $current_specialism[2]->name);
        }

        if ( ! empty($current_specialism[3]->name)) {
            $specialism .= ' | '.str_replace('&amp;', '&', $current_specialism[3]->name);
        }

        if ( ! empty($current_specialism[4]->name)) {
            $specialism .= ' | '.str_replace('&amp;', '&', $current_specialism[4]->name);
        }
    ...
    // Controllbar
    $posts = $wpdb->get_results($queryAll);
    $vactype = [1=>0, 2=>0, 3=>0];
    $source = ['hays'=>0, 'hays-response'=>0];
    foreach ($posts as $post) {
        $vactype[get_post_meta($post->ID, 'job_type')[0]]++;
        $source[get_post_meta($post->ID, 'site')[0]]++;
    }
    ...
    if( $field ) {
        foreach( $field['choices'] as $k => $v ) {
            $typeResult[] = array('name'=> $v, 'count'=> $vactype[$k], 'id'=> $k);
        }
    }
    $sourceResult = [
        [
        ...
        [
            'id' => 2,
            'count' => $source['hays-response'],
            'name' => 'hays-response'
        ]
    ];
    file_put_contents(get_template_directory() . '/server/mock-data/data-controlbar-vactype.json',json_encode($typeResult));
    file_put_contents(get_template_directory() . '/server/mock-data/data-controlbar-source.json',json_encode($sourceResult));
    ...
    return [
        ...
        'country'    => $country,
        'industry'   => $industry,
        'specialism' => $specialismAll
    ];
}

function haysLog($var, $name = 'debug', $added = false)
{
    file_put_contents(HAYS_DIR.'/logs/'.$name.'.log', date('d.m.y H:i:s')."  ".print_r($var,true)."\n\n", ($added?8:0));
}